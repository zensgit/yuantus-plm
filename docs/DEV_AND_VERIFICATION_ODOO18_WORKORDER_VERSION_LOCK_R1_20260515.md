...
New optional arguments with **tri-state** semantics for `document_version_id`:

- **Not passed** (sentinel default `_WORKORDER_VERSION_UNSET`): the existing
  lock on the matching row, if any, is preserved untouched. This is the
  backward-compatible path for legacy callers that never knew about
  version-lock.
- **Explicit `None`**: lock columns are cleared
  (`document_version_id=NULL`, `version_locked_at=NULL`,
  `version_lock_source=NULL`).
- **A string**: validated and applied as a new lock.

`version_lock_source: str = "manual"` — must be one of `manual` / `eco_apply`
/ `backfill`. Unknown → `ValueError`.

Validation, fail-closed:

- `document_version_id` (when set) must resolve to an `ItemVersion`. Missing
  → `ValueError`.
- The resolved version's `item_id` must equal the link's `document_item_id`.
  Mismatch → `ValueError`.

The HTTP router uses Pydantic's `model_fields_set` to only forward the
`document_version_id` keyword when it was actually present in the request
body, so a legacy POST payload (no `document_version_id` key at all) keeps
any existing lock intact. A request body that explicitly sends
`"document_version_id": null` clears the lock.
...
revision `aa1b2c3d4e7b0`, down-revision `f4a5b6c7d8e9` (the actual repo head
prior to this PR — `z1b2c3d4e7a5` was a branch point, not the tip). Adds the
three columns and the `ix_meta_workorder_document_links_document_version_id`
index. The upgrade is idempotent (checks for existing columns and indexes).
Downgrade drops the index and the three columns.

After the rebase, `alembic heads` reports a single head
(`aa1b2c3d4e7b0`). The full chain was exercised end-to-end against a
fresh SQLite database via `alembic upgrade head`, and the new
`document_version_id`, `version_locked_at`, `version_lock_source`
columns are confirmed present on `meta_workorder_document_links`.

Note: `alembic upgrade head --sql` (offline / SQL-emission mode) is
**not** the gate. A pre-existing migration
(`migrations/versions/e5c1f9a4b7d2_add_eco_stage_sla_hours.py`) calls
`sa.inspect(bind)` against the offline-mode `MockConnection`, which
fails with `NoInspectionAvailable`. This breaks `--sql` for the whole
chain in this repo today, independently of this PR. Use the live-DB
upgrade as the verification path until that historical migration is
fixed.
...
- `test_workorder_doc_pack_upsert_legacy_payload_preserves_existing_lock`
- `test_workorder_doc_pack_upsert_explicit_none_clears_lock`
...
### Router behavior tests (new)
(`src/yuantus/meta_engine/tests/test_parallel_tasks_workorder_docs_router_behavior.py`)

- `test_post_workorder_doc_link_passes_document_version_id_through`
- `test_post_workorder_doc_link_omitted_version_field_not_forwarded`
- `test_post_workorder_doc_link_explicit_null_forwarded_to_clear`
- `test_export_passes_require_locked_versions_flag_through`
- `test_export_defaults_require_locked_versions_to_false`
- `test_export_maps_unlocked_value_error_to_409`

## 9. Verification Commands

```bash
PYTHONPATH=src .venv/bin/python -m alembic heads
# Verify the full chain runs against a live (test) DB. --sql/offline mode is
# intentionally NOT used here: a pre-existing migration
# (migrations/versions/e5c1f9a4b7d2_add_eco_stage_sla_hours.py) uses
# sa.inspect(bind) which fails under offline MockConnection. Live upgrade is
# the gate until that historical migration is fixed.
cp alembic.ini /tmp/alembic_test.ini && \
  sed -i.bak 's|sqlalchemy.url = .*|sqlalchemy.url = sqlite:///\/tmp\/yuantus_alembic_test.db|' /tmp/alembic_test.ini && \
  rm -f /tmp/yuantus_alembic_test.db && \
  PYTHONPATH=src .venv/bin/python -m alembic -c /tmp/alembic_test.ini upgrade head
