# Document Sync Reconciliation Router Exception Chaining - Development and Verification

Date: 2026-05-13

## 1. Goal

Preserve original service exceptions when
`document_sync_reconciliation_router.py` maps document-sync reconciliation
lookup failures to existing API-facing `404` responses.
...
- reconciliation job summary failures preserve `ValueError` as
  `HTTPException.__cause__`
- reconciliation site status failures preserve `ValueError` as
  `HTTPException.__cause__`
...
- `py_compile`: passed
- focused document-sync reconciliation exception-chaining contract + ownership contract: 10 passed
...
- Confirm existing document-sync reconciliation route ownership and
