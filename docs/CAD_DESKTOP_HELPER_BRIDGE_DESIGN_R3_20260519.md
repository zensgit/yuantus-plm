...
| R3.3 | 2026-05-20 | S1 实现期 micro-amend。Shared 目标框架收敛为 `net46;net6.0-windows`；`InstallId.GetOrCreate()` 原子 primitive 明确归属 S1，S3 只消费该 primitive 组装 Mutex / session 启动；S1 增补 RegistryView 默认 Registry64、multipart-capable `PostContentAsync`、25 项 mandatory Shared contract tests + reviewer-hardening tests。总工时 15.5 → 16 天。 |
| R3.4 | 2026-05-20 | S2 实现期 micro-amend。新增 `clients/cad-desktop-helper/Detector/` 的 `yuantus-cad-detector`（`net6.0-windows` exe，发布时 self-contained）与 `Detector.Tests`；Detector 使用 S1 的只读注册表抽象，扫描 AutoCAD/Plant 3D/ZWCAD/GstarCAD/SolidWorks 的 HKLM Registry64+Registry32 视图，输出 `schema_version=1.0` JSON 或 table；CI 扩展 Windows `.NET` gate 同时跑 Shared 与 Detector。S2 仍不包含 helper/Kestrel/Mutex/session/token bootstrap。 |
