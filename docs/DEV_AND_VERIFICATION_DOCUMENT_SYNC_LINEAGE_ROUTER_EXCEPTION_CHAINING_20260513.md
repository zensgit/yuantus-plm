# Document Sync Lineage Router Exception Chaining - Development and Verification

Date: 2026-05-13

## 1. Goal

Preserve original service exceptions when `document_sync_lineage_router.py` maps
document-sync lineage lookup failures to existing API-facing `404` responses.
...
- site lineage lookup failures preserve `ValueError` as
  `HTTPException.__cause__`
- job snapshot lineage lookup failures preserve `ValueError` as
  `HTTPException.__cause__`
...
- `py_compile`: passed
- focused document-sync lineage exception-chaining contract + ownership contract: 10 passed
...
- Confirm existing document-sync lineage route ownership and source-declaration
