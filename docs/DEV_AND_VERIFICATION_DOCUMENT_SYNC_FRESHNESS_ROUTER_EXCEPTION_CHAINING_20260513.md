# Document Sync Freshness Router Exception Chaining - Development and Verification

Date: 2026-05-13

## 1. Goal

Preserve the original service exception when `document_sync_freshness_router.py`
maps a site freshness lookup failure to the existing API-facing `404` response.
...
- site freshness lookup failures preserve `ValueError` as
  `HTTPException.__cause__`
- the source keeps exactly one `404 from exc` raise in this router
...
- `py_compile`: passed
- focused document-sync freshness exception-chaining contract + ownership contract: 9 passed
...
- Confirm the `ValueError` mapping path uses `from exc`.
- Confirm existing document-sync freshness route ownership and
