# DEV AND VERIFICATION - Document Sync Router Decomposition R3 Replay Audit

Date: 2026-04-24

## 1. Scope

R3 moves the C27 document-sync replay/audit read surface from the legacy
...
mirror operations, drift, lineage, retention, and freshness/watermark endpoints.

## 2. Design

The new `document_sync_replay_audit_router.py` follows the staged decomposition
pattern from R1 and R2:
...
`test_document_sync_replay_audit_router_contracts.py` pins:

- all four moved routes are owned by `document_sync_replay_audit_router`
...
- the R3 router source declares exactly the four intended C27 paths

The router decomposition portfolio contract now includes the new R3 contract
...
- No movement of drift, lineage, retention, or freshness/watermark routes.
...
1. R4: drift/snapshots router
2. R5: baseline/lineage router
3. R6: checkpoints/retention router
4. R7: freshness/watermarks router
5. R8: core site/job/mirror router and legacy shell closeout
