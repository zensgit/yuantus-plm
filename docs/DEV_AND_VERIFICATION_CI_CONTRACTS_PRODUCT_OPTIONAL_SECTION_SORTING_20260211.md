# CI Contracts: Product + Optional Section Sorting (2026-02-11)
...
Both sections are validated by primary path (first backticked path per bullet), must be unique,
and must be lexicographically sorted.
...
- Updated CI contracts wiring:
  - `.github/workflows/ci.yml` now includes the new contract test
- Reordered section entries:
  - `docs/DELIVERY_DOC_INDEX.md` `## Optional` section adjusted to sorted primary path order
- Indexed this evidence document in:
  - `docs/DELIVERY_DOC_INDEX.md` `## Development & Verification`
