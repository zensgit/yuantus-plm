...
|---|---|---|---|
| P0-⑧a Method script sandbox | 🔨 OPEN — **taskbook authored** (`p0-8a-method-sandbox-taskbook-20260702.md`, PR pending owner ratification); impl gated on ratification + its own opt-in | claude / `docs/p0-8a-method-sandbox-taskbook` | There are **four** execution surfaces: two raw `exec` paths (`business_logic/executor.py:63`, `services/method_service.py:65`) plus two unrestricted `importlib` module paths (`executor.py:79`, `method_service.py:90`). Build a **shared sandbox adapter and switch all four entries in the same slice** — fixing only one path leaves a bypass. Resource limits + execution audit; `METHOD_RPC_ENABLED` fail-closed + admin-only for RPC until the separate RPC identity defect is fixed. |
