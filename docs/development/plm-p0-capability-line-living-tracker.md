...
| P0-① Notification delivery + subscriptions | ✅ RELIABILITY CUT DONE — durable NotificationOutbox/Delivery + worker; subscriptions/digest/channel expansion remain gated follow-ups. | codex / `codex/p0-1-notification-impl` | The generic event layer is still trigger-only. Current implementation writes durable logical notification rows + per-recipient delivery rows in the caller transaction, then drains delivery rows through `notification-worker`. Default adapter is no-I/O `null`; SMTP is explicit config only. Scope supports explicit recipients; role/subscription expansion, digest coalescing, IM/webhook, and notification-center UI are next opt-in slices. |
