# DEV AND VERIFICATION - Document Sync Router Decomposition R6 Retention

Date: 2026-04-24

## 1. Scope

R6 moves the C36 document-sync checkpoints/retention read surface from the
...
mirror operations, and freshness/watermark endpoints.

## 2. Design

The new `document_sync_retention_router.py` follows the staged decomposition
pattern from R1-R5:
...
- Error mapping: existing `ValueError -> 404` behavior preserved for
  site-specific checkpoints reads
...
`test_document_sync_retention_router_contracts.py` pins:

- all four moved routes are owned by `document_sync_retention_router`
...
- the R6 router source declares exactly the four intended C36 paths

The router decomposition portfolio contract now includes the new R6 contract
...
- No movement of freshness/watermark routes.
...
1. R7: freshness/watermarks router
2. R8: core site/job/mirror router and legacy shell closeout
