...
- decision expiry / renewal governance 已经进入可用状态，已经补齐 contract、export evidence、history 续签动作。
- 继续补 `proof-consumer adoption evidence`，证明 UI/support/handoff 真的消费这条
  operator proof contract。
- `conversion rule / external conversion routing evidence` 按 reference 对齐度推进。
...
1. 把 `cad capabilities`、`viewer readiness`、`cad_bom export bundle`、
   `proof acknowledgement / waiver trail`、`checkout gate warn|block` 继续压成统一
   operator/integration 验证包。
2. 在 `proof-consumer adoption` 场景中补齐真实消费与交接证据（UI/support/handoff）。
3. 做决策链可观测性补充：续签提醒、操作动作与证据消费路径。
4. 补参考 reproducibility 决策，解决 `references/` 缺位问题。
