...
当前最关键的一步是把 `cad capabilities` 正式合入主线。

合入后下一步才应该考虑：

- `cad_bom` summary / review / reimport surfacing
- bbox / LOD / callback evidence
...
1. 合并 `file-cad capabilities` 与 `checkout gate strictness` 两条当前分支。
2. 把 `cad capabilities`、`viewer readiness`、`checkout gate warn|block` 写进统一 operator/integration 验证包。
3. 补一条 reference reproducibility 决策，解决 `references/` 缺位问题。
4. 只再开一个 bounded increment，并要求它同时满足 contract test、runbook、verification evidence。
