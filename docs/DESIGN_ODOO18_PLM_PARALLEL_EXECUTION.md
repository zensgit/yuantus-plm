...
### Decision
- `C26/C27/C28` 不再停留在“仅 staging 已验证”，而是已经完成面向 `main` 的 fast-forward rehearsal。
- 当前 fourth-stage greenfield 批次已经具备进入真实主线合并窗口的条件。

### Why
- `feature/codex-c26c27c28-staging` 相对 `main` 是纯快进关系。
- 在 rehearsal 分支 `feature/codex-merge-rehearsal-c26c27c28` 上执行 `--ff-only` 后，没有出现手工冲突。
...
### Decision
- 接受 `C26/C27/C28` 在 `main` 上的短稳定窗口。

### Why
- `main` 上再次重跑：
  - fourth-stage greenfield targeted pack
  - unified stack full
- 结果与 candidate stack / post-merge 初次验证保持一致，没有新增功能回归。

### Result
- targeted greenfield rerun on `main`: `222 passed, 82 warnings in 2.12s`
- unified stack rerun on `main`: `440 passed, 156 warnings in 12.63s`

## Next Claude Batch C29-C31

### Why
- `C26/C27/C28` 已经完成主线合并和稳定窗口确认。
- 下一批如果继续并行，最稳的路径仍然是延续同一组三个 greenfield 子域，而不是引入新的跨域热文件。

### Task Boundaries
- `C29`
  - 只允许扩展 `box` 子域内部 capacity / compliance / export helpers
- `C30`
  - 只允许扩展 `document_sync` 子域内部 drift / snapshot / export helpers
- `C31`
  - 只允许扩展 `cutted_parts` 子域内部 benchmark / quote / export helpers
...
### Decision
- `C29/C30/C31` 不再停留在“仅 staging 已验证”，而是已经完成面向 `main` 的 fast-forward rehearsal。
- 当前 fifth-stage greenfield 批次已经具备进入真实主线合并窗口的条件。

### Why
- `feature/codex-c29c30c31-staging` 相对 `main` 是纯快进关系。
- 在 rehearsal 分支 `feature/codex-merge-rehearsal-c29c30c31` 上执行 `--ff-only` 后，没有出现手工冲突。
- rehearsal 分支上的 unified stack full 结果与候选栈一致，说明候选栈进入主线后的测试面没有新增不稳定因素。
...
### Decision
- 接受 `C29/C30/C31` 在 `main` 上的短稳定窗口。

### Why
- `main` 上再次重跑：
  - fifth-stage greenfield targeted pack
  - unified stack full
- 结果与 candidate stack / post-merge 初次验证保持一致，没有新增功能回归。

### Result
- targeted greenfield rerun on `main`: `267 passed, 98 warnings in 2.67s`
- unified stack rerun on `main`: `485 passed, 172 warnings in 14.52s`

## Next Claude Batch C32-C34

### Why
- `C29/C30/C31` 已经完成主线合并和稳定窗口确认。
- 下一批如果继续并行，最稳的路径仍然是延续同一组三个 greenfield 子域，而不是引入新的跨域热文件。

### Task Boundaries
- `C32`
  - 只允许扩展 `box` 子域内部 policy / exception / export helpers
- `C33`
  - 只允许扩展 `document_sync` 子域内部 baseline / lineage / export helpers
- `C34`
  - 只允许扩展 `cutted_parts` 子域内部 variance / recommendation / export helpers
...
### Decision
- `C35/C36/C37` 不再停留在“仅 staging 已验证”，而是已经完成面向 `main` 的 fast-forward rehearsal。
- 当前 seventh-stage greenfield 批次已经具备进入真实主线合并窗口的条件。

### Why
- `feature/codex-c35c36c37-staging` 相对 `main` 是纯快进关系。
- 在 rehearsal 分支 `feature/codex-merge-rehearsal-c35c36c37` 上执行 `--ff-only` 后，没有出现手工冲突。
- rehearsal 分支上的 unified stack full 结果与候选栈一致，说明候选栈进入主线后的测试面没有新增不稳定因素。
...
### Decision
- 接受 `C35/C36/C37` 在 `main` 上的短稳定窗口。

### Why
- `main` 上再次重跑：
  - seventh-stage greenfield targeted pack
  - unified stack full
- 两组结果与 candidate stack / post-merge 初次验证保持一致，没有新增功能回归。

### Result
- targeted greenfield rerun on `main`: `364 passed, 130 warnings in 3.09s`
- unified stack rerun on `main`: `582 passed, 204 warnings in 13.92s`

## Next Claude Batch C38-C40

### Why
- `C35/C36/C37` 已经完成主线合并和稳定窗口确认。
- 下一批如果继续并行，最稳的路径仍然是延续同一组三个 greenfield 子域，而不是引入新的跨域热文件。

### Task Boundaries
- `C38`
  - 只允许扩展 `box` 子域内部 allocation / custody / export helpers
- `C39`
  - 只允许扩展 `document_sync` 子域内部 freshness / watermark / export helpers
- `C40`
  - 只允许扩展 `cutted_parts` 子域内部 alert / outlier / export helpers
...
### Decision
- `C38/C39/C40` 不再停留在“仅 staging 已验证”，而是已经完成面向 `main` 的 fast-forward rehearsal。
- 当前 eighth-stage greenfield 批次已经具备进入真实主线合并窗口的条件。

### Why
- `feature/codex-c38c39c40-staging` 相对 `main` 是纯快进关系。
- 在 rehearsal 分支 `feature/codex-merge-rehearsal-c38c39c40` 上执行 `--ff-only` 后，没有出现手工冲突。
- rehearsal 分支上的 unified stack full 结果与候选栈一致，说明候选栈进入主线后的测试面没有新增不稳定因素。
...
### Decision
- `C38/C39/C40` 的主线合并结果已完成一次短稳定窗口复核。
...
### Result
- targeted greenfield rerun on `main`: `417 passed, 146 warnings in 5.57s`
- unified stack rerun on `main`: `635 passed, 220 warnings in 13.73s`

## Next Claude Batch C41-C43

### Why
- `C38/C39/C40` 已经完成主线合并和稳定窗口确认。
- 下一批如果继续并行，最稳的路径仍然是延续同一组三个 greenfield 子域，而不是引入新的跨域热文件。

### Task Boundaries
- `C41`
  - 只允许扩展 `box` 子域内部 occupancy / turnover / export helpers
- `C42`
  - 只允许扩展 `document_sync` 子域内部 lag / backlog / export helpers
- `C43`
  - 只允许扩展 `cutted_parts` 子域内部 throughput / cadence / export helpers
