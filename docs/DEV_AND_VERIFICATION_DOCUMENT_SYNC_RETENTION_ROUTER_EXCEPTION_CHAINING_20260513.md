# Document Sync Retention Router Exception Chaining - Development and Verification

Date: 2026-05-13

## 1. Goal

Preserve the original service exception when `document_sync_retention_router.py`
maps a site checkpoints lookup failure to the existing API-facing `404`
response.
...
- site checkpoints lookup failures preserve `ValueError` as
...
- `py_compile`: passed
- focused document-sync retention exception-chaining contract + ownership contract: 9 passed
...
- Confirm existing document-sync retention route ownership and
