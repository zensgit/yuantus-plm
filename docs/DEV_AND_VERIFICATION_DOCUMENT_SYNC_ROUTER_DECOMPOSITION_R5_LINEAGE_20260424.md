# DEV AND VERIFICATION - Document Sync Router Decomposition R5 Lineage

Date: 2026-04-24

## 1. Scope

R5 moves the C33 document-sync baseline/lineage read surface from the legacy
...
mirror operations, retention, and freshness/watermark endpoints.

## 2. Design

The new `document_sync_lineage_router.py` follows the staged decomposition
pattern from R1-R4:
...
- Error mapping: existing `ValueError -> 404` behavior preserved for
  job/site-specific lineage reads
...
`test_document_sync_lineage_router_contracts.py` pins:

- all four moved routes are owned by `document_sync_lineage_router`
...
- the R5 router source declares exactly the four intended C33 paths

The router decomposition portfolio contract now includes the new R5 contract
...
- No movement of retention or freshness/watermark routes.
...
1. R6: checkpoints/retention router
2. R7: freshness/watermarks router
3. R8: core site/job/mirror router and legacy shell closeout
