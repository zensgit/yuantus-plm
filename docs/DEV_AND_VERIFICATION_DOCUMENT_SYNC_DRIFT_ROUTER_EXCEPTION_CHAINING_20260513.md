# Document Sync Drift Router Exception Chaining - Development and Verification

Date: 2026-05-13

## 1. Goal

Preserve original service exceptions when `document_sync_drift_router.py` maps
document-sync drift lookup failures to the existing API-facing `404` responses.
...
- site snapshot lookup failures preserve `ValueError` as
  `HTTPException.__cause__`
- job drift lookup failures preserve `ValueError` as `HTTPException.__cause__`
...
- `py_compile`: passed
- focused document-sync drift exception-chaining contract + ownership contract: 10 passed
...
- Confirm existing document-sync drift route ownership and source-declaration
