...
## Run VERIFY-ALL-DEDUP-MT-20260213-143456

- 时间：`2026-02-13 14:34:56`（本机）
- 环境：
...
## Run STRICT-GATE-RECENT-PERF-BOUND-CHECKS-AND-TYPE-MATRIX-CONTRACTS-20260220

- 时间：`2026-02-20`（GitHub Actions + 本机）
- 目标（1+2）：
  - recent perf 输入增加更严格上限校验（`recent_perf_audit_limit <= 100`，`recent_perf_max_run_age_days <= 100`）。
  - 新增独立 workflow dispatch 输入类型矩阵契约测试，防止 `boolean/choice/string` 类型回退。

### 变更

- commit `51b96d0`：
  - `.github/workflows/strict-gate.yml`
    - 在 `Validate recent perf audit inputs` 中新增：
      - `recent_perf_audit_limit <= 100`
      - `recent_perf_max_run_age_days <= 100`
  - 新增测试：`src/yuantus/meta_engine/tests/test_strict_gate_workflow_dispatch_input_type_contracts.py`
    - 专门锁定 `workflow_dispatch` 输入类型矩阵：
      - boolean：`run_demo/run_perf_smokes/run_recent_perf_audit/recent_perf_fail_if_*`
      - choice：`recent_perf_conclusion`（`any|success|failure`）
      - string：`recent_perf_audit_limit/recent_perf_max_run_age_days`
  - `.github/workflows/ci.yml`
    - contracts step 增加上述新测试，并保持路径排序
  - `src/yuantus/meta_engine/tests/test_strict_gate_workflow_contracts.py`
    - 增加上限校验提示文案契约断言
  - `docs/RUNBOOK_STRICT_GATE.md`
    - recent perf 输入范围说明更新为：
      - `recent_perf_audit_limit: 1..100`
      - `recent_perf_max_run_age_days: 0..100`
...
- run `22227001429`（`2026-02-20T14:00:55Z`）：
  ...
  - 链接：`https://github.com/zensgit/yuantus-plm/actions/runs/22227001429`

- run `22227054135`（`2026-02-20T14:02:29Z`）：
...
## Run STRICT-GATE-PARALLEL-AUTOMATION-AND-SKIP-REDUCTION-20260221

- 时间：`2026-02-21`（GitHub Actions + 本机）
- 目标（并行 1+2+3）：
  - 增加 strict-gate recent perf audit 自动回归脚本（失败/成功双 dispatch 自动断言）。
  - 在 Job Summary 输出 recent perf audit 跳过原因与 artifact 可用性。
  - 收敛 validation-fail 路径下的 `always()` 步骤，避免无效构建产物上传。

### 变更

- commit `0b2e0c9`：
  - `.github/workflows/strict-gate.yml`
    - `Run strict gate report` 新增 `id: strict_gate_report`
    - 以下步骤改为 `if: always() && steps.strict_gate_report.outcome != 'skipped'`：
      - `Build strict gate perf summary`
      - `Build strict gate perf trend`
      - `Write report to job summary`
      - `Upload strict gate report`
      - `Upload strict gate perf summary`
      - `Upload strict gate perf trend`
      - `Upload strict gate logs`
    - `Download artifacts hints` 增加：
      - `Recent perf audit skipped reason`
      - artifact 可用性标记
      - 条件化下载提示（无 artifact 时给出 not generated 提示）
  - 新增脚本：`scripts/strict_gate_recent_perf_audit_regression.sh`
    - 自动触发并校验：
      - invalid case：`recent_perf_audit_limit=101`
      - valid case：`recent_perf_audit_limit=10`
    - 自动下载并校验 `strict_gate_perf_download.json`
  - `docs/RUNBOOK_STRICT_GATE.md`
    - 增加新脚本使用说明与输出说明
    - 明确 validation-fail 时哪些 artifacts 不生成
  - `src/yuantus/meta_engine/tests/test_strict_gate_workflow_contracts.py`
    - 增加 `strict_gate_report` id、`outcome != 'skipped'`、skip-reason 与 artifact-availability 文案契约
...
- 命令：
  - `.venv/bin/pytest -q src/yuantus/meta_engine/tests/test_strict_gate_workflow_contracts.py src/yuantus/meta_engine/tests/test_ci_shell_scripts_syntax.py src/yuantus/meta_engine/tests/test_strict_gate_workflow_dispatch_input_type_contracts.py src/yuantus/meta_engine/tests/test_ci_contracts_job_wiring.py src/yuantus/meta_engine/tests/test_ci_contracts_ci_yml_test_list_order.py`
- 结果：`9 passed`
...
- 脚本汇总产物：
  - `tmp/strict-gate-artifacts/recent-perf-regression/20260221-105742/STRICT_GATE_RECENT_PERF_AUDIT_REGRESSION.md`

- run `22249050374`（`2026-02-21T02:57:47Z`，`main@0b2e0c9`）：
  - 输入：`run_recent_perf_audit=true`，`recent_perf_audit_limit=101`
  - 结果：`failure`（符合预期）
  - 关键步骤：
    - `Validate recent perf audit inputs`: `failure`
  ...
  - 关键错误：
    - `ERROR: recent_perf_audit_limit must be <= 100 (got: 101)`
  - Download artifacts hints 日志包含：
    - `Recent perf audit skipped reason: Validate recent perf audit inputs=failure`
    - `strict-gate report/perf/log artifacts not generated (report step skipped)`
    - `strict-gate-recent-perf-audit artifact not generated`
  - artifacts API：
    - `total_count: 0`
  - 链接：`https://github.com/zensgit/yuantus-plm/actions/runs/22249050374`

- run `22249064587`（`2026-02-21T02:58:44Z`，`main@0b2e0c9`）：
  - 输入：`run_recent_perf_audit=true`，`recent_perf_audit_limit=10`，`recent_perf_conclusion=success`
  - 结果：`success`（符合预期）
  - 关键步骤：
    - `Validate recent perf audit inputs`: `success`
...
## Run STRICT-GATE-RECENT-PERF-FAILURE-EVIDENCE-PERSISTENCE-20260221

- 时间：`2026-02-21`（本机 + GitHub Actions）
- 目标：修复 regression workflow 在失败路径无可下载 evidence 的问题，并降低 valid case 因 `no metrics` 导致的误失败。

### 变更

- commit `82a6843`：
  - `scripts/strict_gate_recent_perf_audit_regression.sh`
    - 新增参数：`--success-fail-if-no-metrics true|false`（默认 `false`）
    - 新增失败路径 summary 持久化：
      - `STRICT_GATE_RECENT_PERF_AUDIT_REGRESSION.md`
      - `STRICT_GATE_RECENT_PERF_AUDIT_REGRESSION.json`
    - JSON 增加字段：`result`、`failure_reason`、`valid_case.requested_fail_if_no_metrics`
    - 引入 `ERR` trap，确保断言/命令失败时也写 summary
  - `.github/workflows/strict-gate-recent-perf-regression.yml`
    - 增加 `REGRESSION_RUN_CONTEXT.txt`
    - 每个 attempt 结束后若存在 MD/JSON 则复制到 `<run_id>/` 根目录（成功/失败均尝试）
  - 测试增强：
    - `test_strict_gate_recent_perf_audit_regression_script_behavior_contracts.py` 新增失败路径 summary 行为用例
    - 更新脚本/workflow 合同测试

### 本地验证

- 命令：
  - `pytest -q src/yuantus/meta_engine/tests/test_strict_gate_recent_perf_audit_regression_script_behavior_contracts.py src/yuantus/meta_engine/tests/test_strict_gate_recent_perf_audit_regression_script_contracts.py src/yuantus/meta_engine/tests/test_strict_gate_recent_perf_regression_workflow_contracts.py src/yuantus/meta_engine/tests/test_strict_gate_workflow_contracts.py src/yuantus/meta_engine/tests/test_ci_shell_scripts_syntax.py src/yuantus/meta_engine/tests/test_strict_gate_workflow_dispatch_input_type_contracts.py src/yuantus/meta_engine/tests/test_workflow_concurrency_contracts.py src/yuantus/meta_engine/tests/test_ci_contracts_job_wiring.py src/yuantus/meta_engine/tests/test_ci_contracts_ci_yml_test_list_order.py src/yuantus/meta_engine/tests/test_ci_contracts_strict_gate_report_perf_smokes.py src/yuantus/meta_engine/tests/test_readme_runbook_references.py src/yuantus/meta_engine/tests/test_readme_runbooks_sorting_contracts.py src/yuantus/meta_engine/tests/test_readme_runbooks_are_indexed_in_delivery_doc_index.py src/yuantus/meta_engine/tests/test_runbook_index_completeness.py`
- 结果：`20 passed`

### 远端验证

- workflow run `22258508018`（`main@82a6843`）：
  - 结果：`success`
  - 关键步骤：
    - `Run strict-gate recent perf audit regression`: `success`
    - `Upload strict-gate recent perf regression evidence`: `success`
    - `Upload strict-gate recent perf regression raw outputs`: `success`
  - 链接：`https://github.com/zensgit/yuantus-plm/actions/runs/22258508018`

- 下载校验（本机）：
  - evidence artifact：
    - `tmp/strict-gate-artifacts/regression-runs/22258508018/evidence/STRICT_GATE_RECENT_PERF_AUDIT_REGRESSION.md`
    - `tmp/strict-gate-artifacts/regression-runs/22258508018/evidence/STRICT_GATE_RECENT_PERF_AUDIT_REGRESSION.json`
  - raw artifact：
    - `tmp/strict-gate-artifacts/regression-runs/22258508018/raw/REGRESSION_RUN_CONTEXT.txt`
    - `tmp/strict-gate-artifacts/regression-runs/22258508018/raw/attempt-1/STRICT_GATE_RECENT_PERF_AUDIT_REGRESSION.md`
    - `tmp/strict-gate-artifacts/regression-runs/22258508018/raw/attempt-1/STRICT_GATE_RECENT_PERF_AUDIT_REGRESSION.json`

- evidence JSON 关键字段：
  - `result: "success"`
  - `failure_reason: ""`
  - `invalid_case.run_id: "22258509811"`
  - `invalid_case.artifact_count: 0`
...
## Run STRICT-GATE-RECENT-PERF-DISPATCH-VALIDATION-SUMMARY-PERSISTENCE-20260223

- 时间：`2026-02-23`（本机 + GitHub Actions）
- 目标：验证 regression workflow 在 dispatch 输入校验失败时，仍会产出可下载的 evidence/raw artifacts（而非空路径）。
...
- commit `1462137`：
  - `.github/workflows/strict-gate-recent-perf-regression.yml`
    - 新增 helper：
      ...
      - `fail_validation`
    - 对 `regression_attempts` / `regression_retry_delay_sec` / `success_fail_if_no_metrics` 的输入校验失败，统一写 MD/JSON summary 后 `exit 2`
    - `REGRESSION_RUN_CONTEXT.txt` 前移到输入校验前写入
...
- workflow run `22306786190`（`main@1462137`）：
  - 触发参数：
  ...
  - 关键步骤：
    ...
    - `Upload strict-gate recent perf regression evidence`: `success`
    - `Upload strict-gate recent perf regression raw outputs`: `success`
  - 链接：`https://github.com/zensgit/yuantus-plm/actions/runs/22306786190`

- 下载校验：
  - evidence:
    - `tmp/strict-gate-artifacts/regression-runs/22306786190/evidence/STRICT_GATE_RECENT_PERF_AUDIT_REGRESSION.md`
    - `tmp/strict-gate-artifacts/regression-runs/22306786190/evidence/STRICT_GATE_RECENT_PERF_AUDIT_REGRESSION.json`
  - raw:
    - `tmp/strict-gate-artifacts/regression-runs/22306786190/raw/REGRESSION_RUN_CONTEXT.txt`
    - `tmp/strict-gate-artifacts/regression-runs/22306786190/raw/STRICT_GATE_RECENT_PERF_AUDIT_REGRESSION.md`
    - `tmp/strict-gate-artifacts/regression-runs/22306786190/raw/STRICT_GATE_RECENT_PERF_AUDIT_REGRESSION.json`
...
- 新增测试：
  - `src/yuantus/meta_engine/tests/test_workflow_inline_shell_syntax_contracts.py`
    ...
    - 对 ubuntu jobs 的每个 `run:` 脚本：
      ...
      - 本地执行 `bash -n`
- CI 接入：
  - `.github/workflows/ci.yml` contracts step 增加该测试路径
...
- 新增测试：
  ...
  - 覆盖：
    ...
    - 对 workflow 引用到的 `.py` 执行 `py_compile`
- CI 接入：
  - `.github/workflows/ci.yml` contracts step 增加该测试路径（保持排序）
...
- CI run `22308784174`（`main@7015780`）：
  - 结果：`success`
  - 关键 job：
    - `contracts`: `success`
    - `playwright-esign`: `success`
    - `plugin-tests`: `success`
  - 链接：`https://github.com/zensgit/yuantus-plm/actions/runs/22308784174`
- CI run `22310525549`（`main@1982de7`）：
  - 结果：`success`
  - 关键 job：
    - `contracts`: `success`
    - `playwright-esign`: `success`
    - `plugin-tests`: `success`
  - 链接：`https://github.com/zensgit/yuantus-plm/actions/runs/22310525549`
...
- CI run `22310652454`（`main@4708ebe`）：
  - 结果：`success`
  - 关键 job：
    - `contracts`: `success`
    - `playwright-esign`: `success`
    - `plugin-tests`: `success`
  - 链接：`https://github.com/zensgit/yuantus-plm/actions/runs/22310652454`

## Run CI-WORKFLOW-SCHEDULE-CRON-GUARD-20260223

- 时间：`2026-02-23`（本机）
- 目标：防止 workflow 定时任务 cron 配置语法错误或范围错误导致 schedule 静默失效。
...
- 新增测试：
  ...
  - 覆盖：
    - 扫描 workflow `on.schedule[].cron`
    - 校验 5 字段 numeric cron 语法（`*` / list / range / step）
    - 校验字段范围：minute/hour/dom/month/dow
    - 校验同一 workflow 内无重复 cron
- CI 接入：
  - `.github/workflows/ci.yml` contracts step 增加该测试路径（保持排序）
...
- CI run `22312847347`（`main@22d0d53`）：
  - 结果：`success`
  - 关键 job：
    - `contracts`: `success`
    - `playwright-esign`: `success`
    - `plugin-tests`: `success`
  - 链接：`https://github.com/zensgit/yuantus-plm/actions/runs/22312847347`

## Run CI-WORKFLOW-USES-REF-PINNING-GUARD-20260223

- 时间：`2026-02-23`（本机）
- 目标：防止 workflow action 依赖漂移，确保 `uses:` 引用固定在版本标签或 commit SHA。
...
- 新增测试：
  ...
  - 覆盖：
    - 扫描 workflow 全部 `uses:` 引用
    - 强制 `@vX` / `@vX.Y` / `@vX.Y.Z` 或 `@<40-char-sha>`
    - 允许 `./local-action` 与 `docker://...`
    - 禁止 `@main` / `@master` / `@latest` / `@head`
- CI 接入：
  - `.github/workflows/ci.yml` contracts step 增加该测试路径（保持排序）
...
- CI run `22312923023`（`main@33d6b7d`）：
  - 结果：`success`
  - 关键 job：
    - `contracts`: `success`
    - `playwright-esign`: `success`
    - `plugin-tests`: `success`
  - 链接：`https://github.com/zensgit/yuantus-plm/actions/runs/22312923023`

## Run CI-WORKFLOW-PERMISSIONS-POLICY-GUARD-20260224

- 时间：`2026-02-24`（本机）
- 目标：收紧 workflow token 权限基线，避免依赖 GitHub 默认权限或使用过宽权限配置。
...
- CI run `22337410927`（`main@37a5cf8`）：
  - 结果：`success`
  - 关键 job：
    - `contracts`: `success`
    - `playwright-esign`: `success`
...
## Run CI-WORKFLOW-UPLOAD-ARTIFACT-RETENTION-GUARD-20260224

- 时间：`2026-02-24`（本机）
- 目标：统一 artifact 生命周期策略，避免上传产物无限保留导致存储膨胀。
...
- 新增测试：
  ...
  - 覆盖：
    - 扫描 workflow 中所有 `actions/upload-artifact@v4` step
    - 强制 `with.retention-days`
    - 强制保留期为整数且落在 `[1, 30]`
- CI 接入：
  - `.github/workflows/ci.yml` contracts step 增加该测试路径（保持排序）
- workflow 硬化：
  - 为以下 workflow 的全部 artifact 上传步骤统一设置 `retention-days: 14`：
...
- CI run `22338842810`（`main@602674a`）：
  - 结果：`success`
  - 关键 job：
    - `contracts`: `success`
    - `playwright-esign`: `success`
    - `plugin-tests`: `success`
  - 链接：`https://github.com/zensgit/yuantus-plm/actions/runs/22338842810`

## Run CI-WORKFLOW-JOB-TIMEOUT-GUARD-20260224

- 时间：`2026-02-24`（本机）
- 目标：为全部 workflow job 建立显式超时上限，避免异常卡死导致 CI 分钟不可控消耗。
...
- 新增测试：
  ...
  - 覆盖：
    ...
    - 强制每个 job 必须声明 `timeout-minutes`
    - 强制超时值为整数且落在 `[1, 120]`
- CI 接入：
  - `.github/workflows/ci.yml` contracts step 增加该测试路径（保持排序）
- workflow 硬化：
  - 为以下缺失超时的 jobs 增加显式 `timeout-minutes`：
    - `.github/workflows/ci.yml`:
      - `detect_changes_ci: 10`
      - `contracts: 20`
      - `plugin-tests: 30`
      - `playwright-esign: 30`
    - `.github/workflows/regression.yml`:
      - `detect_changes_regression: 10`
...
- CI run `22339012501`（`main@70b2bed`）：
  - 结果：`success`
  - 关键 job：
    - `contracts`: `success`
    - `playwright-esign`: `success`
    - `plugin-tests`: `success`
  - 链接：`https://github.com/zensgit/yuantus-plm/actions/runs/22339012501`
- regression run `22339012511`（`main@70b2bed`）：
  - 结果：`success`
  - 关键 job：
...
## Run CI-WORKFLOW-UPLOAD-ARTIFACT-NAME-GUARD-20260224

- 时间：`2026-02-24`（本机）
- 目标：规范 artifact 命名，避免同一 workflow 内产物同名覆盖和下游索引歧义。
...
- 新增测试：
  ...
  - 覆盖：
    - 扫描 `.github/workflows/*.yml` 中全部 `actions/upload-artifact@v4` steps
    - 强制每个上传 step 必须声明非空 `with.name`
    - 强制同一 workflow 内 artifact name 唯一
- CI 接入：
  - `.github/workflows/ci.yml` contracts step 增加该测试路径（保持排序）
...
- CI run `22341220054`（`main@d2925bf`）：
  - 结果：`success`
  - 关键 job：
    - `contracts`: `success`
    - `playwright-esign`: `success`
    - `plugin-tests`: `success`
  - 链接：`https://github.com/zensgit/yuantus-plm/actions/runs/22341220054`
- regression run `22341220056`（`main@d2925bf`）：
  - 结果：`success`
  - 关键 job：
    - `detect_changes (regression)`: `success`
    - `regression`: `success`
  - 链接：`https://github.com/zensgit/yuantus-plm/actions/runs/22341220056`

## Run CI-WORKFLOW-PERMISSIONS-LEAST-PRIVILEGE-GUARD-20260224

- 时间：`2026-02-24`（本机）
- 目标：把 workflow token 权限进一步收敛到最小读权限，杜绝非必要的 write 权限扩散。
...
- 新增测试：
  ...
  - 覆盖：
    - 强制所有 workflow `permissions.contents == read`
    - 强制所有 workflow 默认 `permissions.actions == read`
    - 仅 allowlist workflow 允许 `permissions.actions == write`
- 当前 allowlist：
...
- CI run `22341315560`（`main@7acc1b3`）：
  - 结果：`success`
  - 关键 job：
    - `contracts`: `success`
    - `playwright-esign`: `success`
    - `plugin-tests`: `success`
  - 链接：`https://github.com/zensgit/yuantus-plm/actions/runs/22341315560`
- regression run `22341315534`（`main@7acc1b3`）：
  - 结果：`success`
  - 关键 job：
    - `detect_changes (regression)`: `success`
    - `regression`: `success`
  - 链接：`https://github.com/zensgit/yuantus-plm/actions/runs/22341315534`

## Run CI-WORKFLOW-CONCURRENCY-BASELINE-GUARD-20260224

- 时间：`2026-02-24`（本机）
- 目标：把并发保护从关键 workflow 覆盖提升到全 workflow 覆盖，避免重复运行浪费 CI 分钟与结果竞争。
...
- 新增测试：
  ...
  - 覆盖：
    - 扫描 `.github/workflows/*.yml` 全部 workflow
    - 强制声明顶层 `concurrency`
    - 强制 `concurrency.group` 同时包含 `github.workflow` 与 `github.ref`
    - 强制 `concurrency.cancel-in-progress: true`
- CI 接入：
  - `.github/workflows/ci.yml` contracts step 增加该测试路径（保持排序）
...
- CI run `22343593550`（`main@9304bf0`）：
  - 结果：`success`
  - 关键 job：
    - `contracts`: `success`
    - `playwright-esign`: `success`
    - `plugin-tests`: `success`
  - 链接：`https://github.com/zensgit/yuantus-plm/actions/runs/22343593550`
- regression run `22343593571`（`main@9304bf0`）：
  - 结果：`success`
  - 关键 job：
    - `detect_changes (regression)`: `success`
    - `regression`: `skipped`（按变更范围门控）
  - 链接：`https://github.com/zensgit/yuantus-plm/actions/runs/22343593571`

## Run CI-WORKFLOW-DISPATCH-INPUT-TYPING-GUARD-20260224

- 时间：`2026-02-24`（本机）
- 目标：统一 workflow_dispatch 输入契约，强制显式类型，避免输入解析与 UI 行为漂移。
...
- 新增测试：
  ...
  - 覆盖：
    ...
    - 强制每个输入声明 `type`
    - 强制类型落在 `string|boolean|choice|number|environment`
- CI 接入：
  - `.github/workflows/ci.yml` contracts step 增加该测试路径（保持排序）
- workflow 修正：
  - `.github/workflows/regression.yml`：
    - `workflow_dispatch.inputs.run_cad_ml` 增加 `type: string`
...
- CI run `22343717312`（`main@6398ede`）：
  - 结果：`success`
  - 关键 job：
    - `contracts`: `success`
    - `playwright-esign`: `success`
    - `plugin-tests`: `success`
  - 链接：`https://github.com/zensgit/yuantus-plm/actions/runs/22343717312`
- regression run `22343717293`（`main@6398ede`）：
  - 结果：`success`
  - 关键 job：
    - `detect_changes (regression)`: `success`
    - `regression`: `success`
...
## Run CI-WORKFLOW-NAME-UNIQUENESS-GUARD-20260224

- 时间：`2026-02-24`（本机）
- 目标：确保仓库内 workflow 顶层 `name` 全局唯一（忽略大小写与多空格差异），避免 Actions 页面同名混淆。
...
- 新增测试：
  ...
  - 覆盖：
    ...
    - 强制顶层 `name` 为非空字符串
    - 强制 workflow 名称全局唯一（case-insensitive + whitespace-normalized）
- CI 接入：
  - `.github/workflows/ci.yml` contracts step 增加该测试路径（保持排序）
...
- CI run `22344227383`（`main@f63187d`）：
  - 结果：`success`
  - 关键 job：
    - `contracts`: `success`
    - `playwright-esign`: `success`
    - `plugin-tests`: `success`
  - 链接：`https://github.com/zensgit/yuantus-plm/actions/runs/22344227383`
- regression run `22344227379`（`main@f63187d`）：
  - 结果：`success`
  - 关键 job：
    - `detect_changes (regression)`: `success`
    - `regression`: `success`
  - 链接：`https://github.com/zensgit/yuantus-plm/actions/runs/22344227379`

## Run CI-WORKFLOW-NEEDS-INTEGRITY-GUARD-20260224

- 时间：`2026-02-24`（本机）
- 目标：校验 workflow job 依赖图完整性，避免 `needs` 引用漂移导致的调度失败。
...
- 新增测试：
  ...
  - 覆盖：
    - 扫描 `.github/workflows/*.yml` 的所有 jobs
    - 强制每个 `needs` 引用必须指向当前 workflow 内存在的 job id
    - 强制禁止自依赖、重复依赖
    - 强制依赖图无环
- CI 接入：
  - `.github/workflows/ci.yml` contracts step 增加该测试路径（保持排序）
...
- CI run `22344358959`（`main@706daac`）：
  - 结果：`success`
  - 关键 job：
    - `contracts`: `success`
    - `playwright-esign`: `success`
    - `plugin-tests`: `success`
  - 链接：`https://github.com/zensgit/yuantus-plm/actions/runs/22344358959`
- regression run `22344358947`（`main@706daac`）：
  - 结果：`success`
  - 关键 job：
    - `detect_changes (regression)`: `success`
    - `regression`: `success`
  - 链接：`https://github.com/zensgit/yuantus-plm/actions/runs/22344358947`

## Run CI-WORKFLOW-DISPATCH-INPUT-METADATA-GUARD-20260224

- 时间：`2026-02-24`（本机）
- 目标：统一 `workflow_dispatch.inputs` 元数据契约，防止手动触发参数的展示与默认值语义漂移。
...
- 新增测试：
  ...
  - 覆盖：
    - 扫描 `.github/workflows/*.yml` 的 `workflow_dispatch.inputs`
    - 强制每个 input 声明非空 `description`
    - 强制每个 input 声明布尔 `required`
    - 对 `required: false` 输入强制声明 `default`
    - 对 `type: choice` 强制 `options` 为非空唯一字符串列表
    - 对 `type: choice`，若声明 `default` 则必须在 `options` 内
- CI 接入：
  - `.github/workflows/ci.yml` contracts step 增加该测试路径（保持排序）
...
- CI run `22344532805`（`main@799f60e`）：
  - 结果：`success`
  - 关键 job：
    - `contracts`: `success`
    - `playwright-esign`: `success`
    - `plugin-tests`: `success`
  - 链接：`https://github.com/zensgit/yuantus-plm/actions/runs/22344532805`
- regression run `22344532766`（`main@799f60e`）：
  - 结果：`success`
  - 关键 job：
    - `detect_changes (regression)`: `success`
    - `regression`: `success`
  - 链接：`https://github.com/zensgit/yuantus-plm/actions/runs/22344532766`

## Run CI-WORKFLOW-NO-JOB-LEVEL-PERMISSIONS-GUARD-20260224

- 时间：`2026-02-24`（本机）
- 目标：禁止 job 级 `permissions` 覆盖，统一以 workflow 顶层权限作为唯一来源，避免局部提权漂移。
...
- 新增测试：
  ...
  - 覆盖：
    - 扫描 `.github/workflows/*.yml` 的全部 jobs
    - 强制每个 job 不得声明 `permissions`
    - 权限控制统一保留在 workflow 顶层
- CI 接入：
  - `.github/workflows/ci.yml` contracts step 增加该测试路径（保持排序）
...
- CI run `22344674163`（`main@bc3938d`）：
  - 结果：`success`
  - 关键 job：
    - `contracts`: `success`
    - `playwright-esign`: `success`
    - `plugin-tests`: `success`
  - 链接：`https://github.com/zensgit/yuantus-plm/actions/runs/22344674163`
- regression run `22344674187`（`main@bc3938d`）：
  - 结果：`success`
  - 关键 job：
    - `detect_changes (regression)`: `success`
    - `regression`: `success`
  - 链接：`https://github.com/zensgit/yuantus-plm/actions/runs/22344674187`

## Run CI-WORKFLOW-PERMISSIONS-SCOPE-ALLOWLIST-GUARD-20260224

- 时间：`2026-02-24`（本机）
- 目标：限制 workflow 顶层 `permissions` scope 集合，避免新增未审计 scope 导致 token 权限面扩大。
...
- 新增测试：
  ...
  - 覆盖：
    - 扫描 `.github/workflows/*.yml`
    - 强制顶层 `permissions` 的 scope 落在 allowlist
    - 当前 allowlist：`actions`、`contents`
- CI 接入：
  - `.github/workflows/ci.yml` contracts step 增加该测试路径（保持排序）
...
- CI run `22345076941`（`main@cc4f35b`）：
  - 结果：`success`
  - 关键 job：
    - `contracts`: `success`
    - `playwright-esign`: `success`
    - `plugin-tests`: `success`
  - 链接：`https://github.com/zensgit/yuantus-plm/actions/runs/22345076941`
- regression run `22345076968`（`main@cc4f35b`）：
  - 结果：`success`
  - 关键 job：
    - `detect_changes (regression)`: `success`
    - `regression`: `success`
  - 链接：`https://github.com/zensgit/yuantus-plm/actions/runs/22345076968`

## Run CI-WORKFLOW-TRIGGER-EVENT-ALLOWLIST-GUARD-20260224

- 时间：`2026-02-24`（本机）
- 目标：限制 workflow 触发事件集合，防止误引入非预期触发器造成噪声执行或安全边界漂移。

### 变更

- 新增测试：
  - `src/yuantus/meta_engine/tests/test_workflow_trigger_event_allowlist_contracts.py`
  - 覆盖：
    - 扫描 `.github/workflows/*.yml`
    - 强制顶层 `on` 为映射
    - 强制事件名落在 allowlist
    - 当前 allowlist：`pull_request`、`push`、`schedule`、`workflow_dispatch`
- CI 接入：
  - `.github/workflows/ci.yml` contracts step 增加该测试路径（保持排序）
...
- CI run `22345216672`（`main@7e83820`）：
  - 结果：`success`
  - 关键 job：
    - `contracts`: `success`
    - `playwright-esign`: `success`
    - `plugin-tests`: `success`
  - 链接：`https://github.com/zensgit/yuantus-plm/actions/runs/22345216672`
- regression run `22345216600`（`main@7e83820`）：
  - 结果：`success`
  - 关键 job：
    - `detect_changes (regression)`: `success`
    - `regression`: `success`
  - 链接：`https://github.com/zensgit/yuantus-plm/actions/runs/22345216600`

## Run CI-WORKFLOW-CONCURRENCY-GROUP-TEMPLATE-GUARD-20260224

- 时间：`2026-02-24`（本机）
- 目标：统一 workflow 并发分组模板，避免不同写法造成并发互斥语义不一致。
...
- 新增测试：
  ...
  - 覆盖：
    - 扫描 `.github/workflows/*.yml`
    - 强制 `concurrency.group` 精确等于 `${{ github.workflow }}-${{ github.ref }}`
    - 强制 `concurrency.cancel-in-progress: true`
- CI 接入：
  - `.github/workflows/ci.yml` contracts step 增加该测试路径（保持排序）
...
- CI run `22346129335`（`main@62d659c`）：
  - 结果：`success`
  - 关键 job：
    - `contracts`: `success`
    - `playwright-esign`: `success`
    - `plugin-tests`: `success`
  - 链接：`https://github.com/zensgit/yuantus-plm/actions/runs/22346129335`
- regression run `22346129313`（`main@62d659c`）：
  - 结果：`success`
  - 关键 job：
    - `detect_changes (regression)`: `success`
    - `regression`: `success`
  - 链接：`https://github.com/zensgit/yuantus-plm/actions/runs/22346129313`

## Run CI-WORKFLOW-DISPATCH-DEFAULT-TYPE-CONSISTENCY-GUARD-20260224

- 时间：`2026-02-24`（本机）
- 目标：约束 `workflow_dispatch` 输入 `default` 与声明 `type` 一致，避免手动触发参数在 UI/脚本间出现类型歧义。
...
- CI run `22346265112`（`main@655aebc`）：
  - 结果：`success`
  - 关键 job：
    - `contracts`: `success`
    - `playwright-esign`: `success`
    - `plugin-tests`: `success`
  - 链接：`https://github.com/zensgit/yuantus-plm/actions/runs/22346265112`
- regression run `22346265111`（`main@655aebc`）：
  - 结果：`success`
  - 关键 job：
    - `detect_changes (regression)`: `success`
    - `regression`: `success`
  - 链接：`https://github.com/zensgit/yuantus-plm/actions/runs/22346265111`

## Run CI-WORKFLOW-UPLOAD-ARTIFACT-MISSING-FILE-POLICY-GUARD-20260224

- 时间：`2026-02-24`（本机）
- 目标：统一 `actions/upload-artifact` 缺失文件策略，避免依赖 action 默认值导致行为漂移。

### 变更

- 新增测试：
  - `src/yuantus/meta_engine/tests/test_workflow_upload_artifact_if_no_files_found_contracts.py`
  - 覆盖：
    - 扫描 `.github/workflows/*.yml` 中全部 `actions/upload-artifact@...` steps
    - 强制声明 `with.if-no-files-found`
    - 强制值在 `warn|error|ignore`
- CI 接入：
  - `.github/workflows/ci.yml` contracts step 增加该测试路径（保持排序）
- workflow 规范化：
  - `.github/workflows/perf-p5-reports.yml`：补齐缺失 `if-no-files-found`
  - `.github/workflows/perf-roadmap-9-3.yml`：补齐缺失 `if-no-files-found`
  - `.github/workflows/regression.yml`：补齐缺失 `if-no-files-found`
  - `.github/workflows/strict-gate-recent-perf-regression.yml`：补齐缺失 `if-no-files-found`
  - `.github/workflows/strict-gate.yml`：补齐缺失 `if-no-files-found`
...
- CI run `22348682522`（`main@ab1c7b6`）：
  - 结果：`success`
  - 关键 job：
    - `contracts`: `success`
    - `playwright-esign`: `success`
    - `plugin-tests`: `success`
  - 链接：`https://github.com/zensgit/yuantus-plm/actions/runs/22348682522`
- regression run `22348682527`（`main@ab1c7b6`）：
  - 结果：`success`
  - 关键 job：
    - `detect_changes (regression)`: `success`
    - `regression`: `success`
  ...
  - 链接：`https://github.com/zensgit/yuantus-plm/actions/runs/22348682527`

## Run CI-WORKFLOW-SETUP-RUNTIME-VERSION-BASELINE-GUARD-20260224

- 时间：`2026-02-24`（本机）
- 目标：统一 workflow 工具链安装版本，避免 `setup-python/setup-node` 漂移造成结果不稳定。

### 变更

- 新增测试：
  - `src/yuantus/meta_engine/tests/test_workflow_setup_runtime_versions_contracts.py`
  - 覆盖：
    - 扫描 `.github/workflows/*.yml` 的全部 steps
    - 强制 `actions/setup-python@v5` 配置 `python-version: "3.11"`
    - 强制 `actions/setup-node@v4` 配置 `node-version: "20"`
- CI 接入：
  - `.github/workflows/ci.yml` contracts step 增加该测试路径（保持排序）
...
- CI run `22354343942`（`main@6e3a35b`）：
  - 结果：`success`
  - 关键 job：
    - `contracts`: `success`
    - `playwright-esign`: `success`
    - `plugin-tests`: `success`
  - 链接：`https://github.com/zensgit/yuantus-plm/actions/runs/22354343942`
- regression run `22354343997`（`main@6e3a35b`）：
  - 结果：`success`
  - 关键 job：
    - `detect_changes (regression)`: `success`
    - `regression`: `success`
    - `cad_ml_quick`: `skipped`
    - `cadgf_preview`: `skipped`
  - 链接：`https://github.com/zensgit/yuantus-plm/actions/runs/22354343997`

## Run CI-WORKFLOW-MANUAL-DISPATCH-PRESENCE-GUARD-20260224

- 时间：`2026-02-24`（本机）
- 目标：确保所有 workflow 都支持手动触发，提升事故处置和按需回归能力。
...
- 新增测试：
  ...
  - 覆盖：
    - 扫描 `.github/workflows/*.yml`
    - 强制 `on.workflow_dispatch` 必须存在
    - 强制 `workflow_dispatch` 为映射（无输入时使用 `{}`）
- CI 接入：
  - `.github/workflows/ci.yml` contracts step 增加该测试路径（保持排序）
...
- CI run `22383963324`（`main@8bad22e`）：
  - 结果：`success`
  - 关键 job：
    - `contracts`: `success`
    - `playwright-esign`: `success`
    - `plugin-tests`: `success`
  - 链接：`https://github.com/zensgit/yuantus-plm/actions/runs/22383963324`
- regression run `22383963293`（`main@8bad22e`）：
  - 结果：`success`
  - 关键 job：
    - `detect_changes (regression)`: `success`
    - `regression`: `success`
...
## Run CI-WORKFLOW-UPLOAD-ARTIFACT-ALWAYS-CONDITION-GUARD-20260224

- 时间：`2026-02-24`（本机）
- 目标：确保 artifact 上传步骤在失败路径仍会执行，避免故障时证据丢失。

### 变更

- 新增测试：
  - `src/yuantus/meta_engine/tests/test_workflow_upload_artifact_always_condition_contracts.py`
  - 覆盖：
    - 扫描 `.github/workflows/*.yml` 的 `actions/upload-artifact@...` steps
    - 强制每个 step 显式声明非空 `if`
    - 强制 `if` 包含 `always()`
- CI 接入：
  - `.github/workflows/ci.yml` contracts step 增加该测试路径（保持排序）
...
- CI run `22391639269`（`main@8a14fb2`）：
  - 结果：`success`
  - 关键 job：
    - `contracts`: `success`
    - `playwright-esign`: `success`
    - `plugin-tests`: `success`
  - 链接：`https://github.com/zensgit/yuantus-plm/actions/runs/22391639269`
- regression run `22391639281`（`main@8a14fb2`）：
  - 结果：`success`
  - 关键 job：
    - `detect_changes (regression)`: `success`
    - `regression`: `success`
    - `cad_ml_quick`: `skipped`
    - `cadgf_preview`: `skipped`
  - 链接：`https://github.com/zensgit/yuantus-plm/actions/runs/22391639281`

## Run CI-WORKFLOW-GIT-DIFF-FETCH-DEPTH-GUARD-20260224

- 时间：`2026-02-24`（本机）
- 目标：保证使用 `git diff` 的 workflow jobs 总是拉取完整历史，避免浅克隆导致的变更范围误判。

### 变更

- 新增测试：
  - `src/yuantus/meta_engine/tests/test_workflow_git_diff_fetch_depth_contracts.py`
  - 覆盖：
    - 扫描 `.github/workflows/*.yml` 的 jobs
    - 识别包含 `git diff` 的 inline run steps
    - 强制同 job 存在 `actions/checkout@...` 且 `with.fetch-depth: 0`
- CI 接入：
  - `.github/workflows/ci.yml` contracts step 增加该测试路径（保持排序）
...
- CI run `22394016044`（`main@7f2bea7`）：
  - 结果：`success`
  - 关键 job：
    - `contracts`: `success`
    - `playwright-esign`: `success`
    - `plugin-tests`: `success`
  - 链接：`https://github.com/zensgit/yuantus-plm/actions/runs/22394016044`
- regression run `22394016027`（`main@7f2bea7`）：
  - 结果：`success`
  - 关键 job：
    - `detect_changes (regression)`: `success`
    - `regression`: `success`
...
## Run CI-WORKFLOW-EXTERNAL-CHECKOUT-REF-GUARD-20260226

- 时间：`2026-02-26`（本机）
- 目标：要求跨仓库 checkout 显式固定 `ref`，避免依赖外部仓库默认分支隐式行为。

### 变更

- 新增测试：
  - `src/yuantus/meta_engine/tests/test_workflow_checkout_external_ref_contracts.py`
  - 覆盖：
    - 扫描 `.github/workflows/*.yml` jobs/steps
...
- workflow 规范化：
  - `.github/workflows/regression.yml`
  - 外部 checkout 增加显式 `ref: main`（`zensgit/cad-ml-platform`、`zensgit/CADGameFusion`）
- CI 接入：
  - `.github/workflows/ci.yml` contracts step 增加该测试路径（保持排序）
...
- 命令：
  - `python3 - <<'PY' ... subprocess.run(['pytest', '-q', *paths], check=True) ... PY`（从 `.github/workflows/ci.yml` 提取 contracts 测试列表并执行）
- 结果：`143 passed, 1 skipped`

### 远端验证

- CI run `22426651591`（`main@5e00eb4`）：
  - 结果：`success`
  - 关键 job：
    - `detect_changes (CI)`: `success`
    - `contracts`: `success`
    - `plugin-tests`: `success`
    - `playwright-esign`: `success`
  - 链接：`https://github.com/zensgit/yuantus-plm/actions/runs/22426651591`
- regression run `22426651584`（`main@5e00eb4`）：
  - 结果：`success`
  - 关键 job：
    - `detect_changes (regression)`: `success`
    - `regression`: `success`
...
## Run CI-WORKFLOW-CHECKOUT-VERSION-BASELINE-GUARD-20260227

- 时间：`2026-02-27`（本机）
- 目标：固定所有 checkout action 版本基线为 `v4`，避免主版本漂移引入兼容性风险。
...
- 新增测试：
  ...
  - 覆盖：
    - 扫描 `.github/workflows/*.yml` jobs/steps
    - 识别 `actions/checkout@...`
    - 强制 `uses` 前缀为 `actions/checkout@v4`
- CI 接入：
  - `.github/workflows/ci.yml` contracts step 增加该测试路径（保持排序）
...
- CI run `22486532864`（`main@8030f8e`）：
  - 结果：`success`
  - 关键 job：
    - `detect_changes (CI)`: `success`
    - `contracts`: `success`
    - `plugin-tests`: `success`
    - `playwright-esign`: `success`
  - 链接：`https://github.com/zensgit/yuantus-plm/actions/runs/22486532864`
- regression run `22486532849`（`main@8030f8e`）：
  - 结果：`success`
  - 关键 job：
    - `detect_changes (regression)`: `success`
    - `regression`: `skipped`
    - `cad_ml_quick`: `skipped`
    - `cadgf_preview`: `skipped`
  - 链接：`https://github.com/zensgit/yuantus-plm/actions/runs/22486532849`

## Run CI-WORKFLOW-EXTERNAL-CHECKOUT-REF-ALLOWLIST-GUARD-20260227

- 时间：`2026-02-27`（本机）
- 目标：固定外部 checkout 的 ref 到每个仓库允许值，避免引用目标漂移。
...
- CI run `22486771493`（`main@266071f`）：
  - 结果：`success`
  - 关键 job：
    - `detect_changes (CI)`: `success`
    - `contracts`: `success`
    - `plugin-tests`: `success`
    - `playwright-esign`: `success`
  - 链接：`https://github.com/zensgit/yuantus-plm/actions/runs/22486771493`
- regression run `22486771474`（`main@266071f`）：
  - 结果：`success`
  - 关键 job：
    - `detect_changes (regression)`: `success`
    - `regression`: `skipped`
    - `cad_ml_quick`: `skipped`
    - `cadgf_preview`: `skipped`
  - 链接：`https://github.com/zensgit/yuantus-plm/actions/runs/22486771474`
