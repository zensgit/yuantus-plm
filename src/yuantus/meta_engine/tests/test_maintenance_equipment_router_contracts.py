...
from yuantus.meta_engine.web import maintenance_equipment_router as equipment_module
...
MOVED_ROUTES: Set[Tuple[str, str]] = {
    ...
    ("GET", "/api/v1/maintenance/equipment/readiness-summary"),
    ("GET", "/api/v1/maintenance/equipment/{equipment_id}"),
    ("POST", "/api/v1/maintenance/equipment/{equipment_id}/status"),
}

EXPECTED_OWNER_MODULE = "yuantus.meta_engine.web.maintenance_equipment_router"
...
def test_moved_routes_are_owned_by_maintenance_equipment_router() -> None:
...
def test_maintenance_equipment_router_is_registered_in_app() -> None:
    app_py = Path(__file__).resolve().parents[4] / "src" / "yuantus" / "api" / "app.py"
    text = app_py.read_text(encoding="utf-8")
    split_pos = text.find("app.include_router(maintenance_equipment_router")
    assert split_pos != -1, "maintenance_equipment_router must be registered in app.py"
...
def test_static_readiness_summary_precedes_dynamic_equipment_route() -> None:
    text = Path(equipment_module.__file__).read_text(encoding="utf-8")
    readiness_idx = text.find('"/equipment/readiness-summary"')
    dynamic_idx = text.find('"/equipment/{equipment_id}"')
    assert readiness_idx != -1
    assert dynamic_idx != -1
    assert readiness_idx < dynamic_idx
