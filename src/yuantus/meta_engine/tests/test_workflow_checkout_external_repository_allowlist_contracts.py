...
_ALLOWED_EXTERNAL_CHECKOUT_REPOSITORIES = {
...
def test_external_checkout_repositories_are_allowlisted() -> None:
    ...
    for workflow in workflows:
        ...
        for job_id, job_payload in jobs.items():
            ...
            for idx, step in enumerate(steps):
                ...
                if normalized not in _ALLOWED_EXTERNAL_CHECKOUT_REPOSITORIES:
                    step_label = step.get("name") or f"step[{idx}]"
                    violations.append(
                        f"{rel} job '{job_id}' {step_label}: external checkout repository "
                        f"{normalized!r} not in allowlist "
                        f"{sorted(_ALLOWED_EXTERNAL_CHECKOUT_REPOSITORIES)}"
                    )

    assert checked > 0, "No external actions/checkout steps found under .github/workflows"
    assert not violations, (
        "External checkout repositories must be explicitly allowlisted:\n"
        + "\n".join(violations)
    )
