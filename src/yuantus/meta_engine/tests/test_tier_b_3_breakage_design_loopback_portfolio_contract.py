...
def test_phase4_route_count_pin_still_lives_at_702():
    ...
    # (P3-A BOM multi-table projection).
    text = _PHASE4_TEST.read_text()
    assert "len(app.routes) == 702" in text, (
        "phase-4 route-count pin (702) must still exist as the "
