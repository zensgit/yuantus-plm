...
from yuantus.meta_engine.web import version_revision_router as revision_module
...
EXPECTED_OWNER_MODULE = "yuantus.meta_engine.web.version_revision_router"
...
def test_moved_routes_are_owned_by_version_revision_router() -> None:
...
def test_moved_routes_are_absent_from_legacy_version_router() -> None:
    ...
    pattern = re.compile(r'@version_router\.(get|post|delete|put|patch)\([^)]*"([^"]+)"', re.DOTALL)
    leaked = [
        (method.upper(), path)
        for method, path in pattern.findall(text)
        if path.startswith("/schemes") or path.startswith("/revision/")
    ]
    assert leaked == []


def test_version_revision_router_is_registered_in_app() -> None:
    app_py = Path(__file__).resolve().parents[4] / "src" / "yuantus" / "api" / "app.py"
    text = app_py.read_text(encoding="utf-8")
    split_pos = text.find("app.include_router(version_revision_router")
    assert split_pos != -1, "version_revision_router must be registered in app.py"
...
def test_static_scheme_collection_routes_precede_for_type_route() -> None:
    text = Path(revision_module.__file__).read_text(encoding="utf-8")
    collection_idx = text.find('"/schemes"')
    for_type_idx = text.find('"/schemes/for-type/{item_type_id}"')
    assert collection_idx != -1
    assert for_type_idx != -1
    assert collection_idx < for_type_idx
