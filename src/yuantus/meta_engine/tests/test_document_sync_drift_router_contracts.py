...
EXPECTED_OWNER_MODULE = "yuantus.meta_engine.web.document_sync_drift_router"
...
def test_moved_routes_are_owned_by_document_sync_drift_router() -> None:
...
def test_document_sync_drift_router_is_registered_in_app() -> None:
    app_py = Path(__file__).resolve().parents[4] / "src" / "yuantus" / "api" / "app.py"
    source = app_py.read_text(encoding="utf-8")

    drift_pos = source.find("app.include_router(document_sync_drift_router")
    assert drift_pos != -1, "document_sync_drift_router must be registered in app.py"
...
def test_each_moved_document_sync_drift_path_is_registered_exactly_once() -> None:
...
def test_moved_document_sync_drift_routes_preserve_tag() -> None:
...
def test_document_sync_drift_router_source_declares_exactly_r4_routes() -> None:
    source = Path(drift_module.__file__).read_text(encoding="utf-8")
    declared_paths = [
        path
        for _method, path in re.findall(
            r'@document_sync_drift_router\.(get|post|delete|put|patch)\("([^"]+)"',
            source,
        )
    ]
