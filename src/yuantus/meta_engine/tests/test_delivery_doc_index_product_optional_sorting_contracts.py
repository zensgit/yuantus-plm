...
def _assert_sorted_unique_primary_paths(section_name: str, section_text: str) -> None:
    paths: list[str] = []
...
def test_product_and_optional_sections_are_sorted_and_unique_by_primary_path() -> None:
    repo_root = _find_repo_root(Path(__file__))
    index_path = repo_root / "docs" / "DELIVERY_DOC_INDEX.md"
    assert index_path.is_file()
    text = _read(index_path)

    product = _extract_h2_section(text, "## Product/UI Integration")
    optional = _extract_h2_section(text, "## Optional")

    _assert_sorted_unique_primary_paths("docs/DELIVERY_DOC_INDEX.md > ## Product/UI Integration", product)
    _assert_sorted_unique_primary_paths("docs/DELIVERY_DOC_INDEX.md > ## Optional", optional)
