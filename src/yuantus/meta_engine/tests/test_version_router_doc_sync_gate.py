...
def test_version_checkout_doc_sync_gate_invalid_maps_400():
    ...
    assert detail.get("context", {}).get("gate_mode") == "block"
...
def test_version_checkout_warn_mode_returns_200_with_warning_headers():
    client, db = _client_with_user_id(7)
    db.get.return_value = None
    ...
    with patch("yuantus.meta_engine.web.version_lifecycle_router.DocumentMultiSiteService") as doc_sync_cls:
        with patch("yuantus.meta_engine.web.version_lifecycle_router.VersionService") as version_cls:
            ...
            resp = client.post(
                ...
                json={
                    "doc_sync_site_id": "site-a",
                    "doc_sync_gate_mode": "warn",
                },
            )

    assert resp.status_code == 200
    assert resp.headers.get("x-doc-sync-gate-verdict") == "warn"
    assert resp.headers.get("x-doc-sync-gate-threshold-hits") == "1"
    doc_sync_cls.return_value.evaluate_checkout_sync_gate.assert_called_once_with(
        item_id="item-1",
        site_id="site-a",
        direction=None,
...
def test_version_checkout_passes_direction_thresholds_to_gate():
    client, db = _client_with_user_id(7)
    db.get.return_value = None
    ...
    with patch("yuantus.meta_engine.web.version_lifecycle_router.DocumentMultiSiteService") as doc_sync_cls:
        with patch("yuantus.meta_engine.web.version_lifecycle_router.VersionService") as version_cls:
            ...
            direction_thresholds = {
                "push": {"pending": 1},
                "pull": {"pending": 0, "failed": 2},
            }
            resp = client.post(
                "/api/v1/versions/items/item-1/checkout",
                json={
                    "doc_sync_site_id": "site-a",
                    "doc_sync_direction": "push",
                    "doc_sync_direction_thresholds": direction_thresholds,
                },
            )
