...
def test_all_workflow_jobs_define_human_readable_names() -> None:
    ...
    for workflow in workflows:
        ...
        for job_id, job_payload in jobs.items():
            ...
            assert isinstance(job_name, str) and job_name.strip(), (
                f"{rel} job '{job_id}' must define non-empty string name"
            )
