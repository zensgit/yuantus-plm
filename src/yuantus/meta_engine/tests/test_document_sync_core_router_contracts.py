...
EXPECTED_OWNER_MODULE = "yuantus.meta_engine.web.document_sync_core_router"
...
def test_moved_routes_are_owned_by_document_sync_core_router() -> None:
...
def test_moved_routes_are_absent_from_legacy_document_sync_router() -> None:
    ...
    moved_paths = {
        "/sites",
        "/sites/{site_id}",
        "/sites/{site_id}/mirror-probe",
        "/sites/{site_id}/mirror-execute",
        "/jobs",
        "/jobs/{job_id}",
        "/jobs/{job_id}/summary",
    }
...
def test_moved_document_sync_core_routes_preserve_tag() -> None:
...
def test_document_sync_core_router_source_declares_exactly_r8_routes() -> None:
    source = Path(core_module.__file__).read_text(encoding="utf-8")
    declared = [
        (method.upper(), path)
        for method, path in re.findall(
            r'@document_sync_core_router\.(get|post|delete|put|patch)\("([^"]+)"',
            source,
        )
    ]
    ...
    assert declared == [
        ...
        ("GET", "/sites"),
        ("GET", "/sites/{site_id}"),
        ("POST", "/sites/{site_id}/mirror-probe"),
        ("POST", "/sites/{site_id}/mirror-execute"),
        ("POST", "/jobs"),
        ("GET", "/jobs"),
        ("GET", "/jobs/{job_id}"),
