"""
Route ownership contracts for BOM Router Decomposition R6: Substitutes.

These contracts pin the mechanical relocation of 3 public /api/v1/bom endpoints
(GET/POST /{bom_line_id}/substitutes and
DELETE /{bom_line_id}/substitutes/{substitute_id}) from bom_router.py to
bom_substitutes_router.py. They protect route ownership, legacy absence,
canonical registration order, duplicate registration, tag preservation, source
declaration order, and the final empty legacy shim state.
"""
...
from yuantus.meta_engine.web import bom_substitutes_router as bom_substitutes_module
...
EXPECTED_OWNER_MODULE = "yuantus.meta_engine.web.bom_substitutes_router"
...
def test_moved_routes_are_owned_by_bom_substitutes_router() -> None:
    """All R6 moved routes must resolve to bom_substitutes_router."""
    app = create_app()
...
def test_moved_routes_are_absent_from_legacy_bom_router() -> None:
    """bom_router.py must not declare @bom_router for substitute paths."""
    ...
    assert not leaked, (
        f"bom_router.py still declares substitute route handlers: {leaked}. "
        f"They must live in bom_substitutes_router.py."
    )


def test_bom_substitutes_router_is_registered_before_empty_legacy() -> None:
    ...
    substitutes_pos = text.find("app.include_router(bom_substitutes_router")
    ...
    assert substitutes_pos != -1, "bom_substitutes_router is not registered in app.py"
    ...
    assert (
        ...
        f"-> bom_substitutes_router -> bom_router; got compare={compare_pos}, "
        f"tree={tree_pos}, children={children_pos}, obsolete_rollup={obsolete_rollup_pos}, "
        f"where_used={where_used_pos}, substitutes={substitutes_pos}, legacy={legacy_pos}."
    )


def test_each_moved_path_is_registered_exactly_once() -> None:
    """FastAPI app must have exactly one route per (method, path) pair for R6 paths."""
    app = create_app()
    entries = _collect_app_routes(app)
...
def test_source_declaration_order_list_add_remove() -> None:
    """Declaration order in bom_substitutes_router.py must preserve the legacy order."""
    source_path = Path(bom_substitutes_module.__file__)
    text = source_path.read_text(encoding="utf-8")

    list_idx = text.find('"/{bom_line_id}/substitutes"')
    add_idx = text.find('@bom_substitutes_router.post(\n    "/{bom_line_id}/substitutes"')
    remove_idx = text.find('"/{bom_line_id}/substitutes/{substitute_id}"')
    assert list_idx != -1, "GET /{bom_line_id}/substitutes literal not found"
    assert add_idx != -1, "POST /{bom_line_id}/substitutes decorator not found"
    assert remove_idx != -1, "DELETE /{bom_line_id}/substitutes/{substitute_id} literal not found"
    assert list_idx < add_idx < remove_idx, (
        "Declaration order in bom_substitutes_router.py must be list -> add -> remove "
        f"(got list={list_idx}, add={add_idx}, remove={remove_idx})."
    )


def test_legacy_bom_router_is_empty_compatibility_shim() -> None:
    """After R6 the legacy bom_router.py stays importable but owns no routes."""
    source_path = Path(bom_legacy_module.__file__)
    text = source_path.read_text(encoding="utf-8")
    assert "bom_router = APIRouter" in text
    assert "@bom_router." not in text
