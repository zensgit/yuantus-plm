...
def list_report_definitions(
    ...
    if not _is_admin(user):
        reports = [r for r in reports if _can_access_report(r, user)]
    return {
        "items": [
            ReportDefinitionResponse(
                id=r.id,
...
@report_router.get("/executions", response_model=Dict[str, Any])
...
@report_router.get("/executions/{execution_id}", response_model=ReportExecutionResponse)
def get_report_execution(
    ...
    if not _can_access_report(report, user):
        raise HTTPException(status_code=403, detail="Permission denied")
    if not _is_admin(user) and report.owner_id != user.id:
        if execution.executed_by_id != user.id:
            raise HTTPException(status_code=403, detail="Permission denied")
