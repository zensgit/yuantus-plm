...
class CheckinManager:
    ...
    def checkin(self, item_id: str, content: bytes, filename: str):
        ...
        preview_job = job_service.create_job(
            "cad_preview", dict(_base), user_id=self.user_id,
            max_attempts=3, dedupe=True,
        )
        geometry_job = job_service.create_job(
            "cad_geometry", dict(_base, target_format="glTF"),
            user_id=self.user_id, max_attempts=3, dedupe=True,
        )
        logger.info(
            "CAD checkin queued jobs item_id=%s file_id=%s preview=%s geometry=%s",
            item_id, native.id, preview_job.id, geometry_job.id,
        )

        # 4. Checkin version (unlocks, syncs native_cad VersionFile)
        return self.version_service.checkin(
