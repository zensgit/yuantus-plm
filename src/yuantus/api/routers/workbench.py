...
_HTML_PATH = Path(__file__).resolve().parents[2] / "web" / "workbench.html"
...
def _load_workbench_html() -> str:
    try:
        ...
        raise HTTPException(status_code=500, detail="workbench.html not found") from exc
...
def workbench_page() -> HTMLResponse:
    return HTMLResponse(_load_workbench_html())
