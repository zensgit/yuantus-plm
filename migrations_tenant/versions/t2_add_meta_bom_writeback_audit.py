"""add meta_bom_writeback_audit table (tenant plane) — Phase-7 BOM write-back

Tenant-plane counterpart of migrations/versions/bom_writeback_audit_001. Same
table (governed audit + single-use replay cache); no cross-schema FKs, so nothing
is stripped relative to the t1 baseline style.

Revision ID: t2_bom_writeback_audit
Revises: t1_initial_tenant_baseline
...
revision: str = "t2_bom_writeback_audit"
down_revision: Union[str, None] = "t1_initial_tenant_baseline"
