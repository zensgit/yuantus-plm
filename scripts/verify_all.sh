  ...
  if [[ -f "$pid_file" ]]; then
    ...
    if [[ -n "$pid" ]] && ps -p "$pid" >/dev/null 2>&1; then
      ...
      while IFS= read -r token; do
        ...
        local value="${token#*=}"
        case "$key" in
          YUANTUS_DATABASE_URL|YUANTUS_DATABASE_URL_TEMPLATE|YUANTUS_IDENTITY_DATABASE_URL|YUANTUS_TENANCY_MODE|YUANTUS_SCHEMA_MODE|YUANTUS_AUTH_MODE|YUANTUS_PLATFORM_ADMIN_ENABLED|YUANTUS_PLATFORM_TENANT_ID|YUANTUS_AUDIT_ENABLED|YUANTUS_CADGF_DEFAULT_EMIT|YUANTUS_DEDUP_VISION_BASE_URL|YUANTUS_DEDUP_VISION_FALLBACK_BASE_URL|YUANTUS_DEDUP_VISION_FALLBACK_PORT)
            if [[ -z "${!key:-}" ]]; then
              export "${key}=${value}"
            fi
            ;;
