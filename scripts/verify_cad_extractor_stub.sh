#!/usr/bin/env bash
# =============================================================================
# CAD Extractor Stub Verification Script
# Verifies external cad_extractor integration and required mode.
# =============================================================================
set -euo pipefail
...
TS="$(date +%s)"
DWG_FILE="/tmp/yuantus_cad_extract_stub_${TS}.dwg"
...
CAD_EXTRACTOR_URL="http://127.0.0.1:${PORT}"

PYTHONUNBUFFERED=1 CAD_EXTRACTOR_PORT="$PORT" "$PY" - <<'PY' &
import json
import os
from http.server import BaseHTTPRequestHandler, HTTPServer

port = int(os.environ.get("CAD_EXTRACTOR_PORT", "0"))
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        if self.path.endswith("/api/v1/extract"):
...
for _ in {1..20}; do
  if $CURL -s "$CAD_EXTRACTOR_URL/health" >/dev/null 2>&1; then
    break
  fi
  sleep 0.2
done
...
if ! $CURL -s "$CAD_EXTRACTOR_URL/health" >/dev/null 2>&1; then
  fail "Extractor stub did not start"
fi
...
echo "=============================================="
echo "CAD Extractor Stub Verification"
echo "BASE_URL: $BASE_URL"
echo "TENANT: $TENANT, ORG: $ORG"
echo "EXTRACTOR: $CAD_EXTRACTOR_URL"
echo "=============================================="
...
IMPORT_RESP="$($CURL -X POST "$API/cad/import" \
  ...
  -F "file=@$DWG_FILE;filename=extract_stub_$TS.dwg" \
...
export ORG="$ORG"
export YUANTUS_CAD_EXTRACTOR_BASE_URL="$CAD_EXTRACTOR_URL"
...
if [[ -n "$TENANCY_MODE_ENV" ]]; then
  export YUANTUS_TENANCY_MODE="$TENANCY_MODE_ENV"
fi
...
with get_db_session() as session:
    ...
    job.worker_id = "cad-extractor-direct"
...
VERIFY=$(RESP_JSON="$ATTR_JSON" "$PY" - <<'PY'
import os, json
...
if attrs.get("part_number") != "EXT-001":
    print("FAIL:part_number")
    raise SystemExit(0)
print("OK")
PY
)
...
echo ""
echo "=============================================="
echo "CAD Extractor Stub Verification Complete"
echo "=============================================="
