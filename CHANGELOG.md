...
- Perf CI guard: validate perf workflow wiring (profiles + config + baseline downloader) via unit test.
...
- Perf CI: narrow `perf-roadmap-9-3` pull request paths filter to reduce noise (focus on harness dependencies).
- Perf CI: remove unused perf gate wrapper from `perf-p5-reports` pull request paths filter.
...
- Perf CI: `perf-roadmap-9-3` now runs both SQLite and Postgres benchmarks, gates against recent baselines, and uploads reports + gate logs as artifacts.
- Roadmap 9.3 perf trend: include DB label column (sqlite/postgres).
